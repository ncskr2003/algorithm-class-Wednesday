...
class DNode:
   ...
   def append(self , new): # 노드 기반 삽입 연산
      #현재 노드(self)의 다음 노드로 새 노드 (new) 삽입
       if new is not None:
         new.next = self.next # new 의 다음 노드는 self 노드의 다음 노드
...
class DLinkedList:                       
    ...
    def getNode(self,pos): # pos 기반 연산
            ...
            if self.head == None: #리스트가 빈 상태
                ...
                for _ in range(pos):
                    if ptr == None:
                        return None # pos가 리스트보다 크기가 큰 경우
                    ptr = ptr.next
                return ptr
    def size(self):
        #리스트의 전체 노드의 개수
